...
class UplinkLossEditor extends StatefulWidget {
  final Global global;

  const UplinkLossEditor(this.global, {super.key});

  @override
  State<UplinkLossEditor> createState() => _UplinkLossEditorState();
}

class _UplinkLossEditorState extends State<UplinkLossEditor> {
  ...
  void _getConfigs() async {
    ...
    GetResponse resp = await getParameters(widget.global, [
      'ConfigsForUplinkLoss',
    ]);
    if (!resp.ok) {
      return;
    }
    _configs = resp.getValue('ConfigsForUplinkLoss');
  ...
  void _fetchLossProfile() async {
    ...
    List<String> profile = resp.getValue('UplinkLossProfile');
  ...
  Future<void> _exportToCSV() async {
    ...
    final fileName = 'UplinkLoss_${_selectedConfigName}_$timeStamp';
